...
class ObjectRecognizer(Node):
	...
	def camera_image_callback(self, message):
		...
		if total_objects > 7:
			self.get_logger().info(f"⚠️ Total object count = {total_objects}. Skipping this frame.")
			return

		for key, value in object_count_dict.items():
			if key not in self.allowed_objects:
				self.get_logger().info(f"🚫 Ignoring unknown object: {key}")
				continue
