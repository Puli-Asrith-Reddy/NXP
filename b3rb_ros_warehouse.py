...
class WarehouseExplore(Node):
	...
	def __init__(self):
		...
		self.allowed_objects = ['banana', 'car', 'clock', 'cup', 'horse', 'plant', 'teddy', 'zebra']
	...
	def get_clusters_dbscan(self,
        map_data,
        map_info,
        shelf_width   = 0.40,   # metres
        shelf_height  = 0.80,   # metres
        tolerance     = 0.11,  # metres
        plot=False):
		"""
		...
		for cid in set(labels):
			...
			if not good_size:
				self.get_logger().debug(f"Cluster {cid} rejected: size mismatch (w={width_m:.3f}m, h={height_m:.3f}m)")
	...
	def global_map_callback(self, message):
		...
		# ---------------------------------------------------------------------- #
		# Filter out frontiers too close to map edges (< 2 m)
		# ---------------------------------------------------------------------- #
		filtered_frontiers = []
		...
		# ---------------------------------------------------------------------- #
		# Choose farthest frontier (ignore obstacle density)
		...
		for fy, fx in filtered_frontiers:
			...
			for gx, gy in self.recent_goals:
				...
				if dist_prev < self.revisit_distance_thresh:
					penalty += (self.revisit_distance_thresh - dist_prev) * 2.0

			# Farthest frontier gets highest score
			score = distance - penalty

			self.get_logger().debug(
				f"Frontier ({fx:.1f},{fy:.1f}) | dist={distance:.2f} | penalty={penalty:.2f} | score={score:.2f}"
			)
	...
	def offset_goal_away_from_shelf(self,x, y, angle_deg, offset=1.5):
	...
	def goal_result_callback(self, future):
		...
		if not self.exploration_complete:
			self.logger.info("⏳ Waiting 2 seconds for SLAM to update map...")
			self.clustering_timer = self.create_timer(
				2.0,  # 2 seconds delay
				self.check_shelf_after_goal_with_clustering
			)
		else:
			...
			if self.in_shelf_mode :
				...
				if not self.visited_qr_side:
					...
					# Exit early so we don't publish now
						...
						# Transform data from (names, probs) to (unique_names, counts)
						final_message = self._transform_shelf_data_for_publishing()
						self.publisher_shelf_data.publish(final_message)
	...
	def check_shelf_after_goal_with_clustering(self):
		"""
		One-shot timer callback to re-run clustering after SLAM has updated.
		This ensures we work with the most up-to-date map.
		"""
		# Cancel the timer so it only runs once
		if hasattr(self, 'clustering_timer'):
			self.clustering_timer.cancel()
			self.clustering_timer = None
		
		# Record last goal position to prevent revisiting
		if self.goal_handle_curr:
			goal_pose = self.goal_handle_curr.goal_request.goal.pose.pose.position
			self.recent_goals.append((goal_pose.x, goal_pose.y))
			if len(self.recent_goals) > self.max_recent_goals:
		...
		# 🧠 Re-run clustering with updated map
		if self.global_map_curr:
			...
			self.get_logger().info(f"📦 Shelf-like clusters detected: {len(clusters)}")
			...
			for i, c in enumerate(clusters):
				...
				self.get_logger().info(
					f"  → Cluster {i+1}: width={w:.2f} m, height={h:.2f} m, center=({wx:.2f}, {wy:.2f})"
				)

			if len(clusters) >= self.shelf_count and not self.exploration_complete:
				self.logger.info(f"📦 Found {len(clusters)} shelves after goal — reselecting based on initial angle.")
	...
	def select_first_shelf_from_initial_angle(self):
		...
		if self.shelf_idx is not None:
			...
			self.exploration_complete = False  #new additions
			self.in_shelf_mode = False #new additions
	...
	def shelf_objects_callback(self, message: WarehouseShelf):
			...
			Compares the new frame with the stored best frame based on:
			1. Number of objects detected.
			2. Sum of probabilities if object count is the same.
			"""
			
			current_names = message.object_name
			current_probs = message.object_count # This is a list of probabilities
			current_len = len(current_names)
			
			# Ensure lists are not empty before summing
			if current_len > 0:
				current_prob_sum = sum(current_probs)
			else:
				current_prob_sum = 0

			best_names = self.shelf_qr_data.object_name
			best_probs = self.shelf_qr_data.object_count # This is the stored list of probabilities
			best_len = len(best_names)
			
			if best_len > 0:
				best_prob_sum = sum(best_probs)
			else:
				best_prob_sum = 0

			# self.get_logger().debug(f"New frame: {current_len} objects, sum={current_prob_sum}. Best frame: {best_len} objects, sum={best_prob_sum}")

			if current_len > best_len:
				# Condition 1: More objects detected. This is better.
				# self.get_logger().info(f"New best frame: More objects ({current_len} > {best_len})") # <-- LOG REMOVED
				self.shelf_qr_data.object_name = current_names
				self.shelf_qr_data.object_count = current_probs
			
			elif current_len == best_len and current_len > 0:
				# Condition 2: Same number of objects. Check probability sum.
				if current_prob_sum > best_prob_sum:
					# This frame is better.
					# self.get_logger().info(f"New best frame: Higher prob sum ({current_prob_sum} > {best_prob_sum})") # <-- LOG REMOVED
					self.shelf_qr_data.object_name = current_names
					self.shelf_qr_data.object_count = current_probs
				# else:
					# self.get_logger().debug("Frame ignored: Same count, lower/equal prob sum.")
			
			# else:
				# self.get_logger().debug("Frame ignored: Fewer objects.")


			# Optional code for populating TABLE GUI (can be left as is)
			"""
			if PROGRESS_TABLE_GUI:
	...
	def _transform_shelf_data_for_publishing(self) -> WarehouseShelf:
		"""
		Converts the stored "best frame" data (with probabilities)
		into the final format (with unique names and object counts)
		and adds a scaled sum of probabilities (sum / 10).
		"""
		# Get the best data we've accumulated
		# e.g., names: ['car', 'car', 'clock']
		# e.g., probs: [95, 88, 75]
		names_list = self.shelf_qr_data.object_name
		probs_list = self.shelf_qr_data.object_count # This list holds probabilities
		
		# Calculate sum of probabilities
		prob_sum = 0
		if probs_list: # Ensure list is not empty
			prob_sum = sum(probs_list)

		# --- ADDED: Scale the probability sum ---
		scaled_prob_sum = int(prob_sum / 10)
		# --- END ADDED ---

		# Manually count unique names using a dictionary
		name_counts_dict = {}
		...
		final_message = WarehouseShelf()
		final_message.object_name = list(name_counts_dict.keys())
		# Convert values (counts) to list of integers
		final_message.object_count = [int(c) for c in name_counts_dict.values()]
		
		# --- MODIFIED ---
		# Add the scaled probability sum as a special entry
		final_message.object_name.append("prob_sum")
		final_message.object_count.append(scaled_prob_sum) # Append the scaled value
		# --- END MODIFIED ---
		
		final_message.qr_decoded = self.shelf_qr_data.qr_decoded

		self.get_logger().info(f"Transforming data for publish: {final_message.object_name} with counts {final_message.object_count}")
